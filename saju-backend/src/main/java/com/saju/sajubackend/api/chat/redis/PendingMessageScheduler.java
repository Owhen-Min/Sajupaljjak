package com.saju.sajubackend.api.chat.redis;
...
public class PendingMessageScheduler implements InitializingBean {
    ...
    public void processPendingMessage() {
        ...
        for (PendingMessage pendingMessage : pendingMessages) {
            redisOperator.claimStream(pendingMessage, consumerName); // claim을 통해 consumer 변경
            try {
                // Stream message 조회
                MapRecord<String, Object, Object> messageToProcess = redisOperator
                        .findStreamMessageById(this.streamKey, pendingMessage.getIdAsString());
                if (messageToProcess == null) {
                    log.info("존재하지 않는 메시지");
                } else {
                    // 해당 메시지 에러 발생 횟수 확인
                    int errorCount = (int) redisOperator.getRedisValue("errorCount", pendingMessage.getIdAsString());
                    ...
                    // ack stream
                    redisOperator.ackStream(consumerGroupName, messageToProcess);
                }
            } catch (Exception e) {
                // 해당 메시지 에러 발생 횟수 + 1
