...
public class MatchingQueryDslRepository extends MatchingBaseRepository {
    ...
    public Map<Member, Integer> findMatchingMembers(Long memberId, int maginot, int count) {
        ...
        if (foundFilter == null) {
            throw new BadRequestException(ErrorMessage.FILTER_NOT_FOUND);
        }

        // 3. 80점 이상인 천간과 점수 조회
        Map<CelestialStem, Integer> compatibleStemsMap = getCompatibleCelestialStems(foundMember.getCelestialStem(),
                maginot);
    ...
    public MatchingProfileResponseDto findMatchingMember(Long memberId, Long partnerId) {
        ...
        return queryFactory
                ...
                .leftJoin(score1)
                .on(score1.source.eq(member.celestialStem).and(score1.target.eq(partner.celestialStem))) // 3. 궁합 점수 조회
