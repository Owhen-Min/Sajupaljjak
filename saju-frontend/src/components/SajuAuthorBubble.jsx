...
function SajuUserBubble({ skyElement, size = "normal", onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`cursor-pointer rounded-full border ${style.bg} ${style.text} ${style.border} font-NanumR text-sm font-extrabold flex items-center justify-center ${sizeStyles[size]} shadow`}
    >
      <span>익명의 {skyElement}</span>
    </div>
  );
}
